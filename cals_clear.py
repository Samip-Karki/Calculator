# Just remember that QApplication handles the under the hood
# like event handiling and initilization.(Automatic work)
# And MainWindow handles the graphics and logic.(Manual work)
import PyQt5.QtWidgets as qtw

# Attach the widgets(which are graphics part)
# in the MainWindow part
class MainWindow(qtw.QWidget):
    ...
    def keypad(self):
        ...
        btn_clear = qtw.QPushButton('Clear', clicked=self.clear_clac)
        ...
        btn_plus = qtw.QPushButton('+', clicked=lambda: self.fun_press('+'))
        btn_minus = qtw.QPushButton('-', clicked=lambda: self.fun_press('-'))
        btn_div = qtw.QPushButton('/', clicked=lambda: self.fun_press('/'))
        btn_mult = qtw.QPushButton('*', clicked=lambda: self.fun_press('*'))
    ...
    def num_press(self, key_number):
        self.tmp_nums.append(key_number)
        temp_string = ''.join(self.tmp_nums)
        if self.fin_nums:
            self.result_field.setText(''.join(self.fin_nums) + temp_string)
        else:
            self.result_field.setText(temp_string)

    def fun_press(self, operator):
        temp_string = ''.join(self.tmp_nums)
    ...
    def func_result(self):
        fin_string = ''.join(self.fin_nums) + ''.join(self.tmp_nums)
        try:
            result = eval(fin_string)
            fin_string += '='
            fin_string += str(result)
        except Exception as e:
            print(e)
            fin_string = 'Error Occupred'
        finally:
            # the result on screen should be string
